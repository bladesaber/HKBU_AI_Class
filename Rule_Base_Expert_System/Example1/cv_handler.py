...
class Fact:
    ...
    def __str__(self):
        s = self.fact + '|'.join(list(map(str, self.about)))
        return s
...
def generate_contour_facts(lines):
    '''可以认为这里生成 knowledge representation'''
    line_facts = []
    angle_facts = []
...
class FactGenerator:
    def __init__(self, contour_dict):
        ...
        self.handled_facts = {}
        # self.file = open('D:\HKBU_AI_Classs\Rule_Base_Expert_System\Memory/facts/facts.txt', 'w+')
    ...
    def generate_fact(self):
        # contour_dict 包含轮廓组， 单张图片可能有多个图形
...
#     handler = image_Handler('D:\HKBU_AI_Classs\Rule_Base_Expert_System/test/test00.png')
